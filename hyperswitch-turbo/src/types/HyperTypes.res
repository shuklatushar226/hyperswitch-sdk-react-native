...
let parseAllSavedPaymentMethods = (paymentMethodsDict: Js.Dict.t<Js.Json.t>) => {
  let savedPMJson = paymentMethodsDict->Js.Dict.get("paymentMethods")->Belt.Option.getWithDefault(Js.Json.null)
  let savedPMJsonArr = savedPMJson->Js.Json.decodeArray->Belt.Option.getWithDefault([])

  Belt.Array.map(savedPMJsonArr, item =>
    item->Js.Json.decodeObject->Belt.Option.getWithDefault(Js.Dict.empty())->parseSavedPaymentMethod
  )
}

let parseResponseFromNativeModule = (responseDict: Js.Dict.t<Js.Json.t>) => {
  let type_ = responseDict->getString("type", "")
  let code = responseDict->getString("code", "")
  let message = responseDict->getString("message", "")
